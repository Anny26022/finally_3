...
import { calcWeightedRewardRisk } from "../../lib/calculations";
...
import { useAnalyticsCalculations } from "../../hooks/calculations/useAnalyticsCalculations";
import { formatCurrency, formatPercentage, formatWinRate, formatExpectancy, formatProfitFactor, calcAllocationPercentage } from "../../lib/calculations";
...
export const PerformanceMetrics: React.FC<PerformanceMetricsProps> = ({ trades, isEditing = false }) => {
  ...
  const accountingCalculations = useAccountingCalculations(filteredTrades);
  const { displayName } = useAccountingMethodDisplay();

  // Use centralized analytics calculations for better performance and consistency
  const analyticsCalculations = useAnalyticsCalculations(filteredTrades, [], portfolioSize, useCashBasis);



  // Use centralized analytics calculations for consistency
  const performanceMetrics = analyticsCalculations.performanceMetrics;
  ...
  const cashPercentage = 100 - openAndPartialTrades.reduce((sum, t) => {
    ...
    if (t.positionStatus === 'Partial') {
      // Calculate allocation based on remaining open quantity only
      const remainingInvestedAmount = (t.openQty || 0) * (t.avgEntry || 0);
      remainingAllocation = calcAllocationPercentage(remainingInvestedAmount, portfolioSize);
    } else {
      // For fully open positions, use full allocation
      remainingAllocation = t.allocation || 0;
    }
