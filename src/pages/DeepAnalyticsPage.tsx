...
const DeepAnalyticsPage: React.FC = () => { // Renamed component
                ...
                if (trade.positionStatus === 'Closed' || trade.positionStatus === 'Partial') {
                    ...
                    if (exitDates.length > 0 && trade.date) {
                        try {
                            ...
                            console.warn('Error calculating accrual holding days for trade:', trade.id, error);
    ...
    const analytics = useMemo(() => {
        ...
        if (useCashBasis) {
            ...
            closedTrades.forEach(trade => {
                ...
                if (!tradeGroups.has(originalId)) {
                    tradeGroups.set(originalId, { ...trade, accountingPL: 0, exits: [], exitDates: [] });
                }
                const group = tradeGroups.get(originalId);
                group.accountingPL += pl;
                group.exits.push(trade);

                // Collect exit dates for holding days calculation
                if (trade._cashBasisExit?.date) {
                    group.exitDates.push(trade._cashBasisExit.date);
                }
            });

            // Calculate proper holding days for each grouped trade
            groupedTrades = Array.from(tradeGroups.values()).map(group => {
                // Calculate holding days from entry date to latest exit date
                let calculatedHoldingDays = group.holdingDays || 0; // fallback to original

                if (group.exitDates.length > 0 && group.date) {
                    try {
                        // Find the latest exit date
                        const latestExitDate = group.exitDates.reduce((latest, current) => {
                            return new Date(current) > new Date(latest) ? current : latest;
                        });

                        // Calculate days between entry and latest exit
                        ...
                        // Ensure non-negative
                        calculatedHoldingDays = Math.max(0, calculatedHoldingDays);
                    } catch (error) {
                        console.warn('Error calculating holding days for grouped trade:', error);
                        // Keep original holdingDays as fallback
                    }
                }
        ...
        // ✅ FIXED: Handle undefined/NaN holdingDays values
        const avgWinHold = totalWinningTrades > 0 ?
            winningTrades.reduce((sum, trade) => {
                const holdingDays = trade.holdingDays || 0;
                return sum + (isNaN(holdingDays) ? 0 : holdingDays);
            }, 0) / totalWinningTrades : 0;

        const avgLossHold = totalLosingTrades > 0 ?
            losingTrades.reduce((sum, trade) => {
                const holdingDays = trade.holdingDays || 0;
                return sum + (isNaN(holdingDays) ? 0 : holdingDays);
            }, 0) / totalLosingTrades : 0;
        ...
        // Calculate Ratios (use correct function signatures)
        // ✅ FIXED: Pass daily returns array to calculateSharpeRatio, not scalar values
        const sharpeRatio = calculateSharpeRatio(dailyReturnsArray, dailyRiskFreeRate);
        const calmarRatio = calculateCalmarRatio(annualizedAverageReturn, maxDrawdown);
        // ✅ FIXED: Pass daily returns array to calculateSortinoRatio with correct parameters
        const sortinoRatio = calculateSortinoRatio(dailyReturnsArray, 0, dailyRiskFreeRate);
    ...
    const topAllocations = useMemo(() => {
        ...
        // For partial positions, use only the remaining open quantity
        const tradesWithAllocation = openAndPartialTrades.map(trade => {
            let calculatedAllocation = 0;
            if (portfolioSize > 0) {
                if (trade.positionStatus === 'Partial') {
                    // For partial positions, calculate allocation based on remaining open quantity
                    const remainingInvestedAmount = (trade.openQty || 0) * (trade.avgEntry || 0);
                    calculatedAllocation = (remainingInvestedAmount / portfolioSize) * 100;
                } else {
                    // For fully open positions, use full position size
                    calculatedAllocation = trade.positionSize ? (trade.positionSize / portfolioSize) * 100 : 0;
                }
            }
            return {
                ...trade,
